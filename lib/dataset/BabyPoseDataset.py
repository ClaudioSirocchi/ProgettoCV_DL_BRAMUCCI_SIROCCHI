    ...
    def evaluate(self, cfg, preds, scores, output_dir, tag,
        ...
        for idx, _kpts in enumerate(preds):
            ...
            for idx_kpt, kpt in enumerate(_kpts):
                ...
                the_img_id = file_name.split('/')[0].replace('pz', '') + file_name.split('/')[1].split('_')[0].rjust(5, '0')

                kpts[int(the_img_id)].append(
                    {
                        'keypoints': kpt[:, 0:3],
                        'score': scores[idx][idx_kpt],
                        'image': int(the_img_id),
                        'area': area
                    }
                )
...
class BabyPoseRescoreDataset(BabyPoseDataset):
    def __init__(self, cfg, dataset):
        BabyPoseDataset.__init__(self, cfg, dataset)
